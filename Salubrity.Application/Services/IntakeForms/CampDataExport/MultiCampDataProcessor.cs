...
namespace Salubrity.Application.Services.IntakeForms.CampDataExport
{
    public class MultiCampDataProcessor
    {
        public ProcessedMultiCampData Process(MultiCampData data)
        {
            ...
            foreach (var campData in data.CampDataList)
            {
                ...
                foreach (var fieldResponse in campData.DtoResponses.SelectMany(r => r.FieldResponses))
                {
                    var fieldId = $"intake_{fieldResponse.FieldId}";
                    if (existingIntakeFieldIds.Add(fieldId))
                    {
                        var sectionName = fieldResponse.Field.SectionName ?? "General";
                        allIntakeFormFields.Add(new FieldDefinition(
                            FieldId: fieldId,
                ...
                // Also process entity responses
                foreach (var fieldResponse in campData.EntityResponses.SelectMany(r => r.FieldResponses))
                {
                    var fieldId = $"intake_{fieldResponse.FieldId}";
                    if (existingIntakeFieldIds.Add(fieldId))
                    {
                ...
                // Build health assessment fields
                foreach (var patientAssessments in campData.HealthAssessmentResponses.Values)
                {
                    foreach (var assessmentResponse in patientAssessments)
                    {
                        foreach (var section in assessmentResponse.Sections)
                        {
                            foreach (var field in section.Fields)
                            {
                                ...
                                if (existingHealthFieldIds.Add(fieldId))
                                {
                                    allHealthAssessmentFields.Add(new FieldDefinition(
                                        FieldId: fieldId,
            ...
            {
                var participantResponses = campData.EntityResponses
                    .GroupBy(r => r.PatientId)
                    .Where(g => g.First().Patient != null)
                    .OrderBy(g => g.First().Patient?.User?.FirstName ?? "")
                    .ToList();

                var dtoResponseLookup = campData.DtoResponses
                    .GroupBy(r => r.PatientId)
                    .ToDictionary(g => g.Key, g => g.ToList());

                var healthAssessmentLookup = campData.HealthAssessmentResponses
                    .ToDictionary(
                        kvp => kvp.Key,
                        kvp => kvp.Value
                ...
                var doctorRecommendationLookup = campData.DoctorRecommendations
                    .GroupBy(r => r.PatientId)
                ...
                {
                    allParticipantRows.Add(new MultiCampParticipantRow
                    {
                        CampName = campData.CampName,
                        CampDate = campData.CampDate,
                        OrganizationName = campData.OrganizationName,
                        ParticipantGroup = participantGroup.ToList(),
                        DtoResponses = dtoResponseLookup.TryGetValue(participantGroup.Key, out var dtos) ? dtos : new List<IntakeFormResponseDetailDto>(),
                        HealthAssessmentResponses = healthAssessmentLookup.TryGetValue(participantGroup.Key, out var health) ? health : new Dictionary<string, string>(),
                        DoctorRecommendation = doctorRecommendationLookup.TryGetValue(participantGroup.Key, out var dr) ? dr : null
                    });
                }
            }
            ...
            return new ProcessedMultiCampData
            {
                ...
                TotalParticipants = allParticipantRows.Count
            };
        }
    ...
    public class ProcessedMultiCampData
    {
        public List<MultiCampParticipantRow> ParticipantRows { get; set; } = [];
    ...
    public class MultiCampParticipantRow
    {
        public string CampName { get; set; } = string.Empty;
        public DateTime CampDate { get; set; }
        public string OrganizationName { get; set; } = string.Empty;
        public List<IntakeFormResponse> ParticipantGroup { get; set; } = [];
        public List<IntakeFormResponseDetailDto> DtoResponses { get; set; } = [];
        public Dictionary<string, string> HealthAssessmentResponses { get; set; } = [];
        public DoctorRecommendationResponseDto? DoctorRecommendation { get; set; }
    }
}